...
AK3_DIR="${HOME}/Kernel/outzip/motorola/AnyKernel3-lineage"
OUTZIPS_DIR="${HOME}/Kernel/outzip/motorola/custom/A13base"
...
muke()
{
	...
	make "$@" "${MAKE_ARGS[@]}" "TARGET_PRODUCT=${CODENAME}_g"
}
...
config_generator()
{
	...
	DFCF="vendor/lineage_${CODENAME}-${SUFFIX}_defconfig"
	if [[ ! -f arch/arm64/configs/$DFCF ]]; then
		inform "Generating defconfig"

		export "${MAKE_ARGS[@]}" "TARGET_BUILD_VARIANT=user" "TARGET_PRODUCT=${CODENAME}_g"

        if [[ ! -f arch/arm64/configs/vendor/lahaina-qgki_defconfig ]]; then
		bash scripts/gki/generate_defconfig.sh lahaina-qgki_defconfig vendor/lahaina_QGKI.config vendor/debugfs.config
        fi
		muke vendor/lahaina-qgki_defconfig vendor/lineage_moto-lahaina.config vendor/lineage_${CODENAME}.config savedefconfig
...
zipper()
{
	...
	zip -r9 `echo "${MOD_NAME}_${CODENAME}"`.zip *
